...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-surface-container-low border border-surface-variant rounded-xl overflow-hidden shadow-xl">
        <div class="p-6 border-b border-surface-variant bg-surface-container-lowest/50 relative overflow-hidden">
            ...
            <p class="font-body-sm text-body-sm text-slate-500 relative z-10">Update client information.</p>
        </div>
        ...
        <form action="{{ route('customers.update', $customer) }}" method="POST" class="p-6 space-y-6">
            ...
            <div class="space-y-4">
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="name" id="name" value="{{ $customer->name }}" required class="w-full bg-white border border-slate-200 text-on-surface rounded-lg px-3 py-2 focus:border-primary-container focus:ring-1 focus:ring-primary-container transition-colors" placeholder="e.g. John Doe">
                </div>

                <div class="space-y-1.5">
                    <label for="phone" class="block font-medium text-slate-700 text-sm">Phone Number</label>
                    <input type="text" name="phone" id="phone" value="{{ $customer->phone }}" class="w-full bg-white border border-slate-200 text-on-surface rounded-lg px-3 py-2 focus:border-primary-container focus:ring-1 focus:ring-primary-container transition-colors" placeholder="e.g. 08123456789">
                </div>

                <div class="space-y-1.5">
                    <label for="address" class="block font-medium text-slate-700 text-sm">Address</label>
                    <textarea name="address" id="address" rows="3" class="w-full bg-white border border-slate-200 text-on-surface rounded-lg px-3 py-2 focus:border-primary-container focus:ring-1 focus:ring-primary-container transition-colors" placeholder="Full address...">{{ $customer->address }}</textarea>
                </div>
            </div>
