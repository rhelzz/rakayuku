        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="font-body-sm text-body-sm text-slate-500">Client: {{ $order->customer->name }} | Deadline: {{ $order->deadline ? \Carbon\Carbon::parse($order->deadline)->format('d M Y') : '-' }}</p>
                </div>
            </div>
        ...
        <div class="lg:col-span-3 space-y-6">
            ...
            <div class="glass-panel rounded-xl border border-slate-200 overflow-hidden">
                <div class="flex border-b border-slate-200 bg-surface-container-low/50">
                    <button @click="activeTab = 'overview'" :class="activeTab === 'overview' ? 'text-primary border-b-2 border-primary bg-primary/5' : 'text-slate-500 hover:text-slate-700'" class="px-6 py-4 text-sm font-semibold transition-all">Overview</button>
                    <button @click="activeTab = 'materials'" :class="activeTab === 'materials' ? 'text-primary border-b-2 border-primary bg-primary/5' : 'text-slate-500 hover:text-slate-700'" class="px-6 py-4 text-sm font-semibold transition-all">Materials (Stock Usage)</button>
                    <button @click="activeTab = 'costs'" :class="activeTab === 'costs' ? 'text-primary border-b-2 border-primary bg-primary/5' : 'text-slate-500 hover:text-slate-700'" class="px-6 py-4 text-sm font-semibold transition-all">Additional Costs</button>
                    <button @click="activeTab = 'payments'" :class="activeTab === 'payments' ? 'text-primary border-b-2 border-primary bg-primary/5' : 'text-slate-500 hover:text-slate-700'" class="px-6 py-4 text-sm font-semibold transition-all">Payments</button>
                </div>
                ...
                <div class="p-6 min-h-[400px]">
                        ...
                        <div class="grid grid-cols-2 gap-8">
                            <div>
                                <h4 class="text-on-surface font-semibold mb-4 flex items-center gap-2">
                                    <span class="material-symbols-outlined text-primary text-[20px]">person</span> Customer Details
                                </h4>
                                <div class="space-y-3">
                                    <div>
                            ...
                            <div>
                                <h4 class="text-on-surface font-semibold mb-4 flex items-center gap-2">
                                    <span class="material-symbols-outlined text-primary text-[20px]">payments</span> Financials
                                </h4>
                                <div class="space-y-3">
                                    <div>
                    ...
                    <div x-show="activeTab === 'materials'" class="space-y-6">
                        ...
                        <form action="{{ route('orders.add-material', $order) }}" method="POST" class="bg-surface-container-high/30 p-4 rounded-xl border border-slate-200 grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                            ...
                            <div class="md:col-span-2 space-y-1.5">
                                <label class="text-[11px] font-label-caps text-slate-500 uppercase">Select Material</label>
                            ...
                            <div class="space-y-1.5">
                                <label class="text-[11px] font-label-caps text-slate-500 uppercase">Quantity</label>
                            ...
                            <button type="submit" class="bg-primary text-white py-2 rounded-lg font-semibold text-sm hover:opacity-90 transition-colors">Add to Project</button>
                        </form>
                        @endif
                        ...
                        <table class="w-full text-left">
                            <thead class="border-b border-slate-200">
                                <tr>
                                    <th class="py-3 font-label-caps text-slate-500 uppercase text-[11px]">Material</th>
                                    <th class="py-3 font-label-caps text-slate-500 uppercase text-[11px] text-right">Qty Used</th>
                                    <th class="py-3 font-label-caps text-slate-500 uppercase text-[11px] text-right">Unit HPP (at usage)</th>
                    ...
                    <div x-show="activeTab === 'costs'" class="space-y-6">
                        ...
                        <form action="{{ route('orders.add-cost', $order) }}" method="POST" class="bg-surface-container-high/30 p-4 rounded-xl border border-slate-200 grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                            @csrf
                            <div class="space-y-1.5">
                                <label class="text-[11px] font-label-caps text-slate-500 uppercase">Type</label>
                            ...
                            <div class="space-y-1.5 md:col-span-1">
                                <label class="text-[11px] font-label-caps text-slate-500 uppercase">Description</label>
                                <input type="text" name="description" required class="w-full bg-white border border-slate-200 text-on-surface rounded-lg px-3 py-2 text-sm" placeholder="e.g. Shipping fee">
                            </div>
                            <div class="space-y-1.5">
                                <label class="text-[11px] font-label-caps text-slate-500 uppercase">Amount (Rp)</label>
                            ...
                            <button type="submit" class="bg-primary text-white py-2 rounded-lg font-semibold text-sm hover:opacity-90 transition-colors">Add Cost</button>
                        </form>
                        @endif
                        ...
                        <table class="w-full text-left">
                            <thead class="border-b border-slate-200">
                                <tr>
                                    <th class="py-3 font-label-caps text-slate-500 uppercase text-[11px]">Type</th>
                                    <th class="py-3 font-label-caps text-slate-500 uppercase text-[11px]">Description</th>
                                    <th class="py-3 font-label-caps text-slate-500 uppercase text-[11px] text-right">Amount</th>
                                </tr>
                            </thead>
                            <tbody class="text-sm">
                    ...
                    <div x-show="activeTab === 'payments'" class="space-y-6">
                        ...
                        <form action="{{ route('orders.pay', $order) }}" method="POST" class="bg-surface-container-high/30 p-4 rounded-xl border border-slate-200 grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                            ...
                            <button type="submit" class="bg-primary text-white py-2 rounded-lg font-semibold text-sm hover:opacity-90 transition-colors">Add Payment</button>
                        </form>
                        @endif
                        ...
                        <table class="w-full text-left">
                            <thead class="border-b border-slate-200">
                                <tr>
                                    <th class="py-3 font-label-caps text-slate-500 uppercase text-[11px]">Date</th>
                                    <th class="py-3 font-label-caps text-slate-500 uppercase text-[11px]">Type</th>
                                    <th class="py-3 font-label-caps text-slate-500 uppercase text-[11px] text-right">Amount</th>
                                </tr>
                            </thead>
        ...
        <div class="space-y-6">
            <div class="glass-panel p-6 rounded-xl border border-slate-200">
                <h4 class="text-on-surface font-semibold mb-6 flex items-center gap-2">
                    <span class="material-symbols-outlined text-primary text-[20px]">history</span> Project Lifecycle
                </h4>
                <div class="space-y-6 relative">
                    ...
                    <div class="relative flex items-start gap-4">
                        ...
                        <div>
                            ...
                            @if($order->status == 'IN_PRODUCTION')
                                <p class="text-[11px] text-primary animate-pulse font-bold">In Progress...</p>
                            @elseif($order->status == 'FINISHED')
                                <p class="text-[11px] text-emerald-600 font-bold">Completed</p>
                            @else
                                <p class="text-[11px] text-slate-500 font-medium">Waiting...</p>
                            @endif
                        </div>
                    </div>
            ...
            @if(count($lowStockInProduction) > 0 && $order->status == 'IN_PRODUCTION')
                <div class="p-4 bg-error-container/10 border border-error/30 rounded-xl">
                    ...
                    <p class="text-[11px] text-slate-500 mb-3">Some materials are running very low and might stall production:</p>
                    <div class="space-y-2">
