...
<div class="space-y-6" x-data="{ showAddModal: false }">
    <div class="flex flex-col gap-4">
        ...
        <div class="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-6">
            ...
            <div class="flex flex-wrap gap-3">
                <a href="{{ route('finance.cashflow.export', request()->all()) }}" class="px-6 py-2.5 bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-xl font-bold hover:shadow-lg hover:shadow-emerald-500/30 transition-all flex items-center gap-2 shadow-md">
    ...
    <div class="bg-surface-container-low rounded-xl p-4 border border-surface-variant mb-6">
        <form action="{{ route('finance.cashflow') }}" method="GET" class="flex flex-wrap gap-4 items-end">
            ...
            <div>
                <a href="{{ route('finance.cashflow') }}" class="px-4 py-2 text-error hover:bg-error-container/20 rounded-xl font-bold transition-colors text-sm h-[38px] flex items-center">Reset</a>
            </div>
            @endif
        </form>
    </div>
    ...
    <div x-show="showAddModal" class="fixed z-[100]" style="display: none; top: 0; right: 0; bottom: 0; left: 0;" x-cloak>
        ...
        <div x-show="showAddModal"
            ...
            <form action="{{ route('finance.cashflow.store') }}" method="POST" class="p-6 space-y-5">
                @csrf
